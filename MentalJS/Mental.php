		...
		for(;;) {
		    ...
		    if($this->chr === self::NEWLINE || $this->chr === self::CARRIAGE_RETURN || $this->chr === self::LINE_SEPARATOR || $this->chr === self::PARAGRAPH_SEPARATOR) {                                                   
	            ...
	            if($this->chr === self::DIGIT_0 && ($next === self::UPPER_X || $next === self::LOWER_X)) {
	                $this->states['hex'] = 2;
	                $this->outputLine = $this->outputLine . '0x';
	                $this->pos+=2;
	                $this->states['len'] = 2;                
	            }                  
	            for(;;) {	            		            		            	
	                ...
	                if($this->states['len'] === 0 && $this->chr === self::DIGIT_0 && ($next >= self::DIGIT_0 && $next <= self::DIGIT_9)) {
	                   ...
	                   continue 1; 
	                    ...
	                    $this->outputLine = $this->outputLine . 'e+';
	                    $this->states['e'] = 1;				                    
	                    $this->pos+=2;
	                    continue 1;
	                ...
	                $this->outputLine = $this->outputLine . charAt($code, $this->pos);
	                $this->pos++;
	                $this->states['len']++;                
				...
				$next3 = charCodeAt($code, $this->pos+3);
				$next4 = charCodeAt($code, $this->pos+4);
				$next5 = charCodeAt($code, $this->pos+5);
				$next6 = charCodeAt($code, $this->pos+6);
				$next7 = charCodeAt($code, $this->pos+7);
				$next8 = charCodeAt($code, $this->pos+8);
				$next9 = charCodeAt($code, $this->pos+9);
				$next10 = charCodeAt($code, $this->pos+10);	
				...
				if($this->chr === self::LOWER_F && $next === self::LOWER_U && $next2 === self::LOWER_N && $next3 === self::LOWER_C && $next4 === self::LOWER_T && $next5 === self::LOWER_I && $next6 === self::LOWER_O && $next7 === self::LOWER_N && !$this->isValidVariablePart($next8) && $next8 !== self::BACKSLASH) {								
					...
					$this->left = 0;
					$this->pos+=8;
					$this->outputLine = $this->outputLine . 'function';											
				//if keyword
				...
				// else keyword
					...
					$this->left = 0;	
					$this->pos+=4;
					$this->outputLine = $this->outputLine . ' else ';								
				// while keyword
					...
					$this->left = 0;	
					$this->pos+=5;	
					$this->outputLine = $this->outputLine . 'while';							
				// switch keyword
					...
					$this->left = 0;	
					$this->pos+=6;	
					$this->outputLine = $this->outputLine . 'switch';							
				// with keyword
					...
					$this->left = 0;	
					$this->pos+=4;
					$this->outputLine = $this->outputLine . 'with';								
				// this keyword
					...
					$this->left = 1;	
					$this->pos+=4;
					$this->outputLine = $this->outputLine . 'this';								
				// true keyword
					...
					$this->left = 1;	
					$this->pos+=4;
					$this->outputLine = $this->outputLine . 'true';								
				// false keyword
					...
					$this->left = 1;	
					$this->pos+=5;
					$this->outputLine = $this->outputLine . 'false';																						
				// NaN keyword
					...
					$this->left = 1;	
					$this->pos+=3;
					$this->outputLine = $this->outputLine . 'NaN';								
				// null keyword
				...
				// undefined keyword
					...
					$this->left = 1;	
					$this->pos+=9;
					$this->outputLine = $this->outputLine . 'undefined';								
				// break keyword
					...
					$this->left = 0;	
					$this->pos+=5;
					$this->outputLine = $this->outputLine . 'break ';								
				// case keyword
				...
				// catch keyword			
					...
					$this->left = 0;	
					$this->pos+=5;
					$this->outputLine = $this->outputLine . 'catch';										
				// continue keyword			
					...
					$this->left = 0;	
					$this->pos+=8;
					$this->outputLine = $this->outputLine . 'continue ';										
				// debugger keyword			
					...
					$this->left = 0;	
					$this->pos+=8;
					$this->outputLine = $this->outputLine . 'debugger';										
				// default keyword			
					...
					$this->left = 0;	
					$this->pos+=7;
					$this->outputLine = $this->outputLine . 'default';										
				// delete keyword			
					...
					$this->left = 0;	
					$this->pos+=6;
					$this->outputLine = $this->outputLine . 'delete ';								
				// do keyword		
					...
					$this->left = 0;	
					$this->pos+=2;
					$this->outputLine = $this->outputLine . 'do ';								
				// finally keyword			
					...
					$this->left = 0;	
					$this->pos+=7;
					$this->outputLine = $this->outputLine . 'finally';								
				// in keyword		
				...
				// Infinity keyword		
					...
					$this->left = 1;	
					$this->pos+=8;
					$this->outputLine = $this->outputLine . 'Infinity';																							
				// instanceof keyword		
					...
					$this->left = 1;	
					$this->pos+=10;
					$this->outputLine = $this->outputLine . ' instanceof ';								
				// new keyword
					...
					$this->left = 0;	
					$this->pos+=3;
					$this->outputLine = $this->outputLine . 'new ';									
				// return keyword			
					...
					$this->left = 0;	
					$this->pos+=6;
					$this->outputLine = $this->outputLine . 'return ';								
				// throw keyword			
					...
					$this->left = 0;	
					$this->pos+=5;
					$this->outputLine = $this->outputLine . 'throw ';								
				// try keyword
					...
					$this->left = 0;	
					$this->pos+=3;
					$this->outputLine = $this->outputLine . 'try';								
				// typeof keyword
					...
					$this->left = 0;	
					$this->pos+=6;
					$this->outputLine = $this->outputLine . 'typeof ';																																																				
				// void keyword
				...
				// length keyword
					...
					for(;;) {												
						...
						if($this->chr === self::BACKSLASH) {
							...
							$next4 = charCodeAt($code,$this->pos+4);
							$next5 = charCodeAt($code,$this->pos+5);
							$unicodeChr1 = charAt($code, $this->pos+2);
							$unicodeChr2 = charAt($code, $this->pos+3);
							...
							if(
								...
								if(!$this->states['first']) {
									if($this->isValidVariable(hexdec($unicodeChr1.$unicodeChr2.$unicodeChr3.$unicodeChr4))) {
										$this->outputLine = $this->outputLine . '\\u'.$unicodeChr1.$unicodeChr2.$unicodeChr3.$unicodeChr4;
										$this->pos+=6;
										continue 1;
									} else {
				...
				if($this->state === 'FunctionStatementCurlyOpen' || $this->state === 'FunctionExpressionCurlyOpen') {
					if($options['rewrite']) {	
						$this->outputLine = $this->outputLine . 'var $arguments$=M.A(arguments);';
					}
				}
